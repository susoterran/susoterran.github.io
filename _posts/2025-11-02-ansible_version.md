...
보통 ansible이라고 한다면 ansible 전체 환경을 하나로 묶은 패키지를 말하며 내부에는 core, galaxy, vault 등의 여러 구성 요소들이 포함된다.
(ansible community package 라고도 부른다)
이 중 ansible-core는 ansible 언어, 런타임, 내장 모듈 세트 등을 가지고 있는 ansible 내의 핵심 프레임워크이다.

마치 ansible은 리눅스 배포판, ansible-core 를 커널에 비유할 수 있다.
리눅스 커널이 C/C++으로 작성된 것과 마찬가지로, ansible-core는 파이썬으로 작성되어 있다.
리눅스 배포판의 버전에 따라 지원하는 리눅스 커널과 C 컴파일러 및 라이브러리의 버전이 달라지듯이, ansible의 버전에 따라 내부적으로 사용하는 ansible-core 버전과 시스템에 요구하는 파이썬 버전이 달라진다.
...
|:--------------------------:|:-------------------:|:--------------------:|:----------------------------:|:-----------------------------------------------------:|
| 14.0.0                     | 개발 중 (배포x)      | 2.21                 |                              |                                                       |
| 13.x                       | 최신 배포판          | 2.20                 | Python 3.12 - 3.14           | Python 3.9 - 3.14 PowerShell 5.1                      |
| 12.x                       | 2025/12 이후 중단    | 2.19                 | Python 3.11 - 3.13           | Python 3.9 - 3.14 PowerShell 5.1                      |
| 11.x                       | 2025/12 이후 중단    | 2.18                 | Python 3.11 - 3.13           | Python 3.8 - 3.13 PowerShell 5.1                      |
